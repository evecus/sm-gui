...
export const api = {
  ...
  UpdateGroup: (id, name, subURL, autoUpdate, intervalHours) => call('UpdateGroup', id, name, subURL, autoUpdate, intervalHours),
  MoveGroup: (id, delta) => call('MoveGroup', id, delta),
  RefreshGroupSubscription: (groupID) => call('RefreshGroupSubscription', groupID),
  DeleteGroup: (id) => call('DeleteGroup', id),
  ImportNodes: (content, groupID) => call('ImportNodes', content, groupID),
  FetchSubscriptionAsGroup: (url) => call('FetchSubscriptionAsGroup', url),
  ...
  UnapplyNode: () => call('UnapplyNode'),
  TestNodeLatency: (id) => call('TestNodeLatency', id),
  TestNodeSpeed: (id) => call('TestNodeSpeed', id),
...
let mockGroups = [{ id: 'default', name: '默认', is_default: true, sub_url: '', auto_update: false, update_interval_hours: 0, last_update: 0 }]
let mockStatus = { running: false, pid: 0 }
let mockTestResults = {} // dev mock: nodeId -> {latency, speed}
...
async function mockCall(method, ...args) {
  ...
  switch (method) {
    ...
    case 'AddGroup': {
      const g = { id: Date.now().toString(), name: args[0], is_default: false, sub_url: '', auto_update: false, update_interval_hours: 0, last_update: 0 }
    ...
    case 'UpdateGroup': {
      const g = mockGroups.find(x => x.id === args[0])
      if (!g) return null
      if (!g.is_default) g.name = args[1]
      g.sub_url = args[2] || ''
      g.auto_update = !!args[3] && !!g.sub_url
      g.update_interval_hours = g.sub_url ? (args[4] || 0) : 0
      return null
    }
    case 'MoveGroup': {
      const i = mockGroups.findIndex(x => x.id === args[0])
      const j = i + args[1]
      if (i > 0 && j > 0 && j < mockGroups.length) {
        [mockGroups[i], mockGroups[j]] = [mockGroups[j], mockGroups[i]]
      }
      return null
    }
    case 'RefreshGroupSubscription': {
      const g = mockGroups.find(x => x.id === args[0])
      if (!g || !g.sub_url) throw new Error('该分组未设置订阅链接')
      g.last_update = Math.floor(Date.now() / 1000)
      return 3
    }
    case 'DeleteGroup': {
    ...
    case 'FetchSubscriptionAsGroup': {
      const maxN = mockGroups.reduce((m, g) => {
        const mt = (g.name || '').match(/^订阅(\d+)$/)
        return mt ? Math.max(m, +mt[1]) : m
      }, 0)
      const g = {
        id: 'sub' + Date.now(), name: `订阅${maxN + 1}`, is_default: false,
        sub_url: args[0], auto_update: false, update_interval_hours: 0, last_update: 0,
      }
      mockGroups.push(g)
      mockNodes.push({ id: Date.now().toString(), name: '订阅节点', protocol: 'vmess', address: 'sub.example.com', port: 443, sub_url: args[0], group_id: g.id })
      return { group: g, count: 1 }
    }
    ...
    case 'UnapplyNode': mockAppliedNodeId = ''; return null
    case 'TestNodeLatency': {
      const ms = 120 + Math.floor(Math.random() * 700)
      mockTestResults[args[0]] = { latency: ms }
      await new Promise(r => setTimeout(r, 800))
      if (Math.random() < 0.1) throw new Error('连接失败')
      return ms
    }
    case 'TestNodeSpeed': {
      const mbps = Math.random() * 80 + 2
      mockTestResults[args[0]] = { ...(mockTestResults[args[0]] || {}), speed: mbps }
      await new Promise(r => setTimeout(r, 1500))
      if (Math.random() < 0.1) throw new Error('下载失败')
      return mbps
    }
    case 'ExportNodeURI': {
