...
// ─── Actions ──────────────────────────────────────────────────────────────
  ...
  const handleFetchSub = async (url) => {
    setLoading(true)
    try {
      // 拉取成功 → 新建「订阅N」分组（最右侧），节点全部放入该分组
      const res = await api.FetchSubscriptionAsGroup(url)
      await loadNodes()
      await loadGroups()
      await loadSettings()
      if (res?.group?.id) setActiveGroupId(res.group.id)
      showToast(`订阅拉取成功，新建分组「${res.group.name}」，共 ${res.count} 个节点`, 'success')
      setShowSubModal(false)
    } catch (e) {
      showToast('订阅拉取失败: ' + (e?.message || e), 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleUnapplyNode = async () => {
    try {
      await api.UnapplyNode()
      setAppliedId('')
      showToast('已取消应用节点', 'success')
    } catch (e) {
      showToast('取消应用失败: ' + (e?.message || e), 'error')
      // 失败时以后端状态为准刷新标记
      loadApplied()
    }
  }
