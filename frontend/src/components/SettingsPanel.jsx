...
const DNS_DEFAULT_PORTS = { udp: 53, tcp: 53, tls: 853, https: 443, quic: 443 }
const dnsDefaultPort = t => DNS_DEFAULT_PORTS[t] || 53
...
const DEFAULT_BUILTIN = {
  ...
  dns_mode: 'redir-host',
  ipv6: false,
  clash_api_disabled: false,
  resolver_dns: '223.5.5.5',
  resolver_dns_backup: '119.29.29.29',
...
// builtin 段深合并：后端送来的嵌套对象若含零值字段（如全新安装漏默认），
// 不整体覆盖前端默认，逐字段兜底
function mergeBuiltin(def, cur) {
  cur = cur || {}
  const mergeObj = (d, c) => ({ ...d, ...(c || {}) })
  return {
    ...def,
    ...cur,
    clash_api: mergeObj(def.clash_api, cur.clash_api),
    singbox_direct: mergeObj(def.singbox_direct, cur.singbox_direct),
    singbox_proxy: mergeObj(def.singbox_proxy, cur.singbox_proxy),
    mihomo_direct: Array.isArray(cur.mihomo_direct) && cur.mihomo_direct.length ? cur.mihomo_direct : def.mihomo_direct,
    mihomo_proxy: Array.isArray(cur.mihomo_proxy) && cur.mihomo_proxy.length ? cur.mihomo_proxy : def.mihomo_proxy,
  }
}
...
export default function SettingsPanel({ settings, onSave }) {
  const [form, setForm] = useState(() => ({
    ...DEFAULTS,
    ...settings,
    // settings 缺 builtin 段（旧版/零值）时逐字段补默认（深合并）
    builtin: mergeBuiltin(DEFAULT_BUILTIN, settings?.builtin),
  }))
  ...
  // DNS 端口失焦：空/非法时补该类型默认端口
  const fixDNSPort = (key, type) => {
    const cur = form.builtin[key].port
    if (cur === '' || cur === null || cur === undefined || Number(cur) < 1) {
      setDNS(key, 'port', dnsDefaultPort(type))
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (!/^(\d{1,3}\.){3}\d{1,3}$|^[0-9a-fA-F:]+$/.test(String(b.resolver_dns).trim())) {
      return alert('解析 DNS 主服务器必须是 IP 地址')
    }
    if (!/^(\d{1,3}\.){3}\d{1,3}$|^[0-9a-fA-F:]+$/.test(String(b.resolver_dns_backup).trim())) {
      return alert('解析 DNS 备用服务器必须是 IP 地址')
    }
    ...
    try {
      await onSave({
        ...
        builtin: {
          ...
          resolver_dns_backup: String(b.resolver_dns_backup).trim(),
  ...
  const trimDNS = d => ({
    ...
    // 端口空/非法归一化为 0，后端按类型补默认端口
    port: Number(d.port) > 0 ? Number(d.port) : 0,
  })
  // 端口显示：0/空 显示为空串，placeholder 提示默认端口
  const dnsPortDisplay = d => (Number(d.port) > 0 ? d.port : '')
            ...
            <div className="settings-section">
              ...
              <div className="settings-row">
                ...
                <span className="settings-hint">
                  以管理员身份运行时注册计划任务（开机后仍以管理员运行）；普通权限时写入注册表 Run 键
                </span>
              </div>
              <div className="settings-row">
                <label className="settings-label">静默启动</label>
                <input
                  type="checkbox"
                  className="settings-check"
                  checked={!!form.silent_start}
                  onChange={e => set('silent_start', e.target.checked)}
                />
                <span className="settings-hint">仅开机自启动时生效：不显示主窗口，只保留任务栏托盘图标；手动启动不受影响</span>
              </div>
            </div>
        ...
        {view === 'conf' && (
          <>
            ...
            <div className="settings-section">
              ...
              <div className="settings-row">
                <label className="settings-label">解析 DNS 主</label>
                <input
                  className="modal-input settings-input"
                ...
                />
                <span className="settings-hint">用于解析 DNS 服务器自身的域名（sing-box 取主；mihomo 对应 default-nameserver）</span>
              </div>
              <div className="settings-row">
                <label className="settings-label">解析 DNS 备用</label>
                <input
                  className="modal-input settings-input"
                  value={form.builtin.resolver_dns_backup}
                  onChange={e => setB('resolver_dns_backup', e.target.value)}
                  placeholder="必须是 IP，如 119.29.29.29"
                />
                <span className="settings-hint">仅 mihomo 生效（default-nameserver 填主 + 备用两个）</span>
              </div>
                ...
                <div className="settings-row">
                  ...
                  <div className="settings-inline">
                    ...
                    <input
                      ...
                      type="number"
                      value={dnsPortDisplay(form.builtin.singbox_direct)}
                      onChange={e => setDNS('singbox_direct', 'port', e.target.value === '' ? '' : Number(e.target.value))}
                      onBlur={() => fixDNSPort('singbox_direct', form.builtin.singbox_direct.type)}
                      placeholder={`${dnsDefaultPort(form.builtin.singbox_direct.type)}（默认）`}
                      title="选填，留空使用该类型默认端口"
                    />
                    <input
                      className="modal-input"
                ...
                <div className="settings-row">
                  ...
                  <div className="settings-inline">
                    ...
                    <input
                      className="modal-input num"
                      type="number"
                      value={dnsPortDisplay(form.builtin.singbox_proxy)}
                      onChange={e => setDNS('singbox_proxy', 'port', e.target.value === '' ? '' : Number(e.target.value))}
                      onBlur={() => fixDNSPort('singbox_proxy', form.builtin.singbox_proxy.type)}
                      placeholder={`${dnsDefaultPort(form.builtin.singbox_proxy.type)}（默认）`}
                      title="选填，留空使用该类型默认端口"
                    />
                    <input
                      className="modal-input"
            ...
            <div className="settings-section">
              ...
              <div className="settings-row">
                <label className="settings-label">启用 clash-api</label>
                <input
                  type="checkbox"
                  className="settings-check"
                  checked={!form.builtin.clash_api_disabled}
                  onChange={e => setB('clash_api_disabled', !e.target.checked)}
                />
                <span className="settings-hint">关闭后生成的配置完全不含 clash-api 字段</span>
              </div>
              {!form.builtin.clash_api_disabled && (
                <>
                  <div className="settings-row">
                    <label className="settings-label">监听地址</label>
                    <select
