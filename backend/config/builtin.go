...
type BuiltinOptions struct {
	...
	CachePath      string           // cache_file 绝对路径（run/cache.db）
...
// 地址必填；端口、路径选填：端口为零值时按类型补默认端口（udp/tcp 53、tls 853、https/quic 443），
// 保证配置到手即用；路径仅 https 类型写入。
...
func dnsDefaultPort(t string) int {
	switch t {
	case "tls":
		return 853
	case "https", "quic":
...
func buildBuiltinMihomo(opts BuiltinOptions, n *node.Node) ([]byte, error) {
	...
	if !b.ClashAPIDisabled {
		cfg["external-controller"] = fmt.Sprintf("%s:%d", b.ClashAPI.Listen, b.ClashAPI.Port)
...
func resolverNS(b BuiltinSettings) []interface{} {
	var out []interface{}
	seen := map[string]bool{}
	for _, s := range []string{b.ResolverDNS, b.ResolverDNSBackup} {
...
func buildBuiltinMihomoDNS(opts BuiltinOptions) map[string]interface{} {
	...
	dns := map[string]interface{}{
		...
		"default-nameserver": resolverNS(*b),
	}
	if b.DNSMode == DNSModeFakeIP {
