...
func nodeToSingBoxOutbound(n node.Node) (map[string]interface{}, error) {	ob := map[string]interface{}{
		"tag":         "proxy",
		"server":      n.Address,
		"server_port": n.Port,
	}
...
// SingBoxOutboundForNode returns the sing-box outbound JSON for a node with
// tag "proxy" (RawOutbound verbatim when available). Exported for the probe
// package, which builds temporary test configs around a single node.
func SingBoxOutboundForNode(n node.Node) (map[string]interface{}, error) {
	if n.RawOutbound != nil {
		ob := n.RawOutbound
		ob["tag"] = "proxy"
		return ob, nil
...
// RemoveNodeFromConfig undoes ApplyNodeToConfig: removes the "proxy" outbound
// (sing-box) / the "proxy" proxies entry (mihomo). References left behind
// (route rules / proxy-groups members) are cleaned where the format allows.
// Used by 取消应用.
func RemoveNodeFromConfig(core, cfgPath string) error {
	if core == CoreMihomo {
		return removeNodeFromMihomoConfig(cfgPath)
	}
	return removeNodeFromSingBoxConfig(cfgPath)
}

// removeNodeFromSingBoxConfig removes the "proxy" outbound from a sing-box JSON config.
func removeNodeFromSingBoxConfig(cfgPath string) error {
	cfg, err := loadJSON(cfgPath)
	if err != nil {
		return err
	}
	...
	if !removed {
		return nil // nothing applied — nothing to do
	}
	cfg["outbounds"] = kept
...
// removeNodeFromMihomoConfig removes the "proxy" proxies entry and every
// proxy-group reference to it from a mihomo YAML config.
func removeNodeFromMihomoConfig(cfgPath string) error {
	cfg, err := loadYAML(cfgPath)
	if err != nil {
		return err
	}
	proxies := getProxies(cfg)
	var keptProxies []interface{}
	removed := false
	for _, pr := range proxies {
		...
		keptProxies = append(keptProxies, pr)
	}
	if !removed {
		return nil
	}
